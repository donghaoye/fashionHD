...
for i in range(num_batch):
    print('[%s] test color transfer: %d / %d' % (opt.id, i+1, num_batch))
    ...
    if use_ftn:
        visualizer.visualize_image_matrix(imgs=imgs_generated_trans, imgs_title=imgs_title, vis_dir=vis_dir, label='trans_color_ftn_%d' % i)
